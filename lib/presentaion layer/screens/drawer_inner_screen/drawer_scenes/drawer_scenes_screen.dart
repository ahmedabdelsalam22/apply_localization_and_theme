...
class DrawerScenesScreen extends StatelessWidget {
  const DrawerScenesScreen({Key? key}) : super(key: key);
    ...
    return Scaffold(
      ...
      body: ListView.builder(
        itemBuilder: (context, index) {
          return DrawerScenesItemList(title: getLang(context, 'living_Room'));
        },
        itemCount: 5,
      ),
    );
...
class DrawerScenesItemList extends StatelessWidget {
  final String title;

  const DrawerScenesItemList({Key? key, required this.title}) : super(key: key);
